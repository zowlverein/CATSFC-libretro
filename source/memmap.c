...
uint32_t FileLoader(uint8_t* buffer, const char* filename, int32_t maxsize)
{
    ...
    do
    {
        FileSize = sceIoRead(ROMFile, ptr, maxsize + 0x200 - (ptr - Memory.ROM));
        sceIoClose(ROMFile);
...
bool LoadSRAM(const char* filename)
{
    ...
    if (size)
    {
        ...
        if (file)
        {
            int len = sceIoRead(file, (unsigned char*)Memory.SRAM, 0x20000);
            sceIoClose(file);
...
bool SaveSRAM(const char* filename)
{
    ...
    if (size && *Memory.ROMFilename)
    {
        ...
        if (file)
        {
            sceIoWrite(file, (unsigned char*)Memory.SRAM, size);
            sceIoClose(file);
